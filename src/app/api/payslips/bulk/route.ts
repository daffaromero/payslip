...
export async function POST(req: NextRequest) {
  try {
    ...
    const { employeeIds, templateId, periodType, startDate, endDate, overtimeHours, bonus, notes } =
      parsed.data
    ...
    // Batch YTD query — avoid N+1
    const ytdGroups = await prisma.payslip.groupBy({
      by: ['employeeId'],
      where: {
        employeeId: { in: employeeIds },
        startDate: { gte: startOfYear, lt: startDateObj },
      },
    ...
    const ytdMap = new Map(
      ytdGroups.map(g => [
        g.employeeId,
        {
          ytdGross: Number(g._sum.ytdGross) || 0,
          ytdPph21: Number(g._sum.ytdPph21) || 0,
        },
      ])
    )
    ...
    // Create payslips in a single transaction
